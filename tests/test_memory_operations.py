...
def test_memory_operations():
    ...
    # If the API server isn't running, skip the integration test to avoid
    # failing in environments without network access.
    try:
        requests.get(base_url, timeout=2)
    except requests.exceptions.RequestException:
        pytest.skip("memory service not available on localhost:5002")
